...
for year in range(2020, 2021):
    ...
    try:
        for i in range(1, 366):
            ...
            filenames = f"A{year}{str(i).zfill(3)}*.nc"
            console.log(f"{date}: {filenames}")
            ...
            ds.cdo_command(f"sellonlatbox,{monitoring_station_lon[0]},{monitoring_station_lon[1]},{monitoring_station_lat[0]},{monitoring_station_lat[1]} selname,chlor_a")
            ds.spatial_mean()
            ds.run()
            # ds.merge()
            df = ds.to_dataframe()
